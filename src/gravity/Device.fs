...
//    let persistentBuffer    = new Direct3D11.Texture2D (backBuffer.Device, backBuffer.Description)
    ...
    let solid (c : Color)   = new Direct2D1.SolidColorBrush (d2dRenderTarget, c.ToColor4 () |> rcolor4)
    ...
    member x.Draw (a : Direct2D1.RenderTarget->Direct2D1.RenderTarget->unit) =
        ...
        d2dRenderTarget.Clear <| Nullable<_> (Color.Black.ToColor4() |> rcolor4)
        let rect = RectangleF (-width / 2.F, -height / 2.F, width, height)
        d2dRenderTarget.DrawBitmap (bitmap, rect |> rrectanglef, 1.F, Direct2D1.BitmapInterpolationMode.Linear)
